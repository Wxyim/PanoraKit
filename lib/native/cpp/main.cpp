...
static jstring new_nullable_string(JNIEnv* env, const char* value) {
  if (value == NULL) return NULL;

  return jni_new_string(env, value);
}

static jstring take_owned_string(JNIEnv* env, char* value) {
  if (value == NULL) return NULL;

  jstring result = jni_new_string(env, value);
  free(value);
...
JNIEXPORT jstring JNICALL Java_com_github_yumelira_yumebox_core_bridge_Bridge_nativeStartRootTun(
  ...
  return new_nullable_string(env, error);
}
...
JNIEXPORT jstring JNICALL Java_com_github_yumelira_yumebox_core_bridge_Bridge_nativeStartHttp(
  ...
  return new_nullable_string(env, listened);
}
...
Java_com_github_yumelira_yumebox_core_bridge_Bridge_nativeInspectCompiledGroups(
  ...
  scoped_string response =
      inspectCompiledGroups(_yaml_text, _profile_dir, (int)exclude_not_selectable);
...
static void call_tun_interface_mark_socket_impl(void* tun_interface, int fd) {
  ...
  env->CallVoidMethod((jobject)tun_interface, (jmethodID)m_tun_interface_mark_socket, (jint)fd);
}
...
static void call_completable_complete_impl(void* completable, const char* exception) {
  ...
  if (exception == NULL) {
    env->CallBooleanMethod((jobject)completable, (jmethodID)m_completable_complete,
                           (jobject)o_unit);
  } else {
    jthrowable _exception = (jthrowable)env->NewObject(
        (jclass)c_clash_exception, (jmethodID)m_clash_exception, (jstring)new_string(exception));

    env->CallBooleanMethod((jobject)completable, (jmethodID)m_completable_complete_exceptionally,
                           (jobject)_exception);
  }
}
...
static void call_fetch_callback_report_impl(void* fetch_callback, const char* status_json) {
  ...
  env->CallVoidMethod((jobject)fetch_callback, (jmethodID)m_fetch_callback_report,
...
static void call_fetch_callback_complete_impl(void* fetch_callback, const char* error) {
  ...
  if (error != NULL) _error = new_string(error);

  env->CallVoidMethod((jobject)fetch_callback, (jmethodID)m_fetch_callback_complete,
                      (jstring)_error);
}
...
static int call_logcat_interface_received_impl(void* callback, const char* payload) {
  ...
  env->CallVoidMethod((jobject)callback, (jmethodID)m_logcat_interface_received,
...
static int open_content_impl(const char* url, char* error, int error_length) {
  ...
  if (env->ExceptionCheck()) {
    ...
    jstring message =
        (jstring)env->CallObjectMethod((jthrowable)exception, (jmethodID)m_get_message);

    if (message == NULL) {
      snprintf(error, error_length, "%s", "unknown");
    } else {
      scoped_string _message = get_string(message);

      snprintf(error, error_length, "%s", (const char*)_message);
    }
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
  ...
  m_completable_complete_exceptionally =
      find_method(c_completable, "completeExceptionally", "(Ljava/lang/Throwable;)Z");
  m_logcat_interface_received =
      find_method(c_logcat_interface, "received", "(Ljava/lang/String;)V");
  ...
  m_open = env->GetStaticMethodID(_c_content, "open", "(Ljava/lang/String;)I");

  o_unit =
      env->GetStaticObjectField(c_unit, env->GetStaticFieldID(c_unit, "INSTANCE", "Lkotlin/Unit;"));
...
JNIEXPORT jstring JNICALL Java_com_github_yumelira_yumebox_core_bridge_Bridge_nativeCompilePreview(
  ...
  scoped_string request = get_string(request_json);

  char* result = compilePreview(request);
...
JNIEXPORT jstring JNICALL Java_com_github_yumelira_yumebox_core_bridge_Bridge_nativeCompileToFile(
  ...
  char* result = compileToFile(request);
