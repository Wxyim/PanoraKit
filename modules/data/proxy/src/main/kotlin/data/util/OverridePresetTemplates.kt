...
enum class OverridePresetTemplateId(val title: String, val summary: String) {
    OfficialMrsCommon(
        title = "官方 MRS 常用分流",
...
enum class OverridePresetRegion(
    ...
    HK(displayName = "香港自动组", groupName = "HK Auto", filter = "(?i)(香港|HK|Hong Kong|🇭🇰)"),
    TW(displayName = "台湾自动组", groupName = "TW Auto", filter = "(?i)(台湾|TW|Taiwan|🇹🇼)"),
    JP(displayName = "日本自动组", groupName = "JP Auto", filter = "(?i)(日本|JP|Japan|东京|大阪|🇯🇵)"),
    SG(displayName = "新加坡自动组", groupName = "SG Auto", filter = "(?i)(新加坡|SG|Singapore|狮城|🇸🇬)"),
    US(
        displayName = "美国自动组",
        groupName = "US Auto",
...
enum class OverridePresetItem(
    ...
    Ads(id = "ads", title = "广告拦截", summary = "启用 category-ads-all 并走 REJECT。"),
    Private(id = "private", title = "私有地址直连", summary = "启用 private 规则集并走 DIRECT。"),
    Google(id = "google", title = "Google", summary = "启用 Google 分流和专属策略组。", isService = true),
    Telegram(
        id = "telegram",
        title = "Telegram",
    ...
    GitHub(id = "github", title = "GitHub", summary = "启用 GitHub 分流和专属策略组。", isService = true),
    Microsoft(
        id = "microsoft",
        title = "Microsoft",
    ...
    Apple(id = "apple", title = "Apple", summary = "启用 Apple 分流和专属策略组。", isService = true),
    YouTube(id = "youtube", title = "YouTube", summary = "启用 YouTube 分流和专属策略组。", isService = true),
    Netflix(id = "netflix", title = "Netflix", summary = "启用 Netflix 分流和专属策略组。", isService = true),
    Spotify(id = "spotify", title = "Spotify", summary = "启用 Spotify 分流和专属策略组。", isService = true),
    OpenAI(id = "openai", title = "OpenAI", summary = "启用 OpenAI 分流和专属策略组。", isService = true),
    Steam(id = "steam", title = "Steam", summary = "启用 Steam 分流和专属策略组。", isService = true),
    Cn(id = "cn", title = "中国大陆直连", summary = "启用 cn 域名和 IP 规则并走 DIRECT。"),
    Proxy(id = "proxy", title = "代理规则集", summary = "启用 proxy 规则集并走 Proxy。"),
    GeolocationNotCn(
        id = "geolocation_not_cn",
        title = "境外地理规则",
        summary = "启用 geolocation-!cn 并走 Proxy。",
    ),
    Match(id = "match", title = "兜底 MATCH", summary = "末尾追加 MATCH,Proxy。"),
}
...
private enum class RuleBehavior(val wireName: String) {
    Domain("domain"),
    IpCidr("ipcidr"),
}
...
private data class ServiceRuleTemplate(val providerId: String, val noResolve: Boolean = false)
...
private val itemProviders =
    mapOf(
        OverridePresetItem.Ads to
            listOf(RuleProviderTemplate("ads_domain", "category-ads-all", RuleBehavior.Domain)),
        OverridePresetItem.Private to
            listOf(RuleProviderTemplate("private_domain", "private", RuleBehavior.Domain)),
        OverridePresetItem.Google to
            listOf(
                RuleProviderTemplate("google_domain", "google", RuleBehavior.Domain),
                RuleProviderTemplate("google_ip", "google", RuleBehavior.IpCidr),
            ),
        OverridePresetItem.Telegram to
            listOf(
        ...
        OverridePresetItem.GitHub to
            listOf(RuleProviderTemplate("github_domain", "github", RuleBehavior.Domain)),
        OverridePresetItem.Microsoft to
            listOf(RuleProviderTemplate("microsoft_domain", "microsoft", RuleBehavior.Domain)),
        OverridePresetItem.Apple to
            listOf(RuleProviderTemplate("apple_domain", "apple", RuleBehavior.Domain)),
        OverridePresetItem.YouTube to
            listOf(RuleProviderTemplate("youtube_domain", "youtube", RuleBehavior.Domain)),
        OverridePresetItem.Netflix to
            listOf(RuleProviderTemplate("netflix_domain", "netflix", RuleBehavior.Domain)),
        OverridePresetItem.Spotify to
            listOf(RuleProviderTemplate("spotify_domain", "spotify", RuleBehavior.Domain)),
        OverridePresetItem.OpenAI to
            listOf(RuleProviderTemplate("openai_domain", "openai", RuleBehavior.Domain)),
        OverridePresetItem.Steam to
            listOf(RuleProviderTemplate("steam_domain", "steam", RuleBehavior.Domain)),
        OverridePresetItem.Cn to
            listOf(
                RuleProviderTemplate("cn_domain", "cn", RuleBehavior.Domain),
                RuleProviderTemplate("cn_ip", "cn", RuleBehavior.IpCidr),
            ),
        OverridePresetItem.Proxy to
            listOf(RuleProviderTemplate("proxy_domain", "proxy", RuleBehavior.Domain)),
        OverridePresetItem.GeolocationNotCn to
            listOf(
                RuleProviderTemplate(
...
private val serviceRules =
    mapOf(
        ...
        OverridePresetItem.GitHub to listOf(ServiceRuleTemplate("github_domain")),
        OverridePresetItem.Microsoft to listOf(ServiceRuleTemplate("microsoft_domain")),
        OverridePresetItem.Apple to listOf(ServiceRuleTemplate("apple_domain")),
        OverridePresetItem.YouTube to listOf(ServiceRuleTemplate("youtube_domain")),
        OverridePresetItem.Netflix to listOf(ServiceRuleTemplate("netflix_domain")),
        OverridePresetItem.Spotify to listOf(ServiceRuleTemplate("spotify_domain")),
        OverridePresetItem.OpenAI to listOf(ServiceRuleTemplate("openai_domain")),
        OverridePresetItem.Steam to listOf(ServiceRuleTemplate("steam_domain")),
    )

private val serviceGroupNames =
    mapOf(
...
private val templateProviderIds =
    itemProviders.values.flatten().map(RuleProviderTemplate::id).toSet()
...
fun defaultOverridePresetTemplateSelection(): OverridePresetTemplateSelection {
    return OverridePresetTemplateSelection(enabledItems = defaultEnabledPresetItems())
}

fun inferPresetTemplateSelection(config: ConfigurationOverride): OverridePresetTemplateSelection {
    ...
    val hasTemplateSignals =
        providerKeys.any(templateProviderIds::contains) ||
            ...
            groupNames.any(
                OverridePresetRegion.entries.map(OverridePresetRegion::groupName).toSet()::contains
            ) ||
    ...
    val inferredRegions =
        OverridePresetRegion.entries.filter { region -> region.groupName in groupNames }.toSet()

    val inferredItems =
        itemOrder.filterTo(linkedSetOf()) { item ->
...
private fun normalizeEnabledItems(enabledItems: Set<OverridePresetItem>): Set<OverridePresetItem> {
...
private fun buildRuleProviders(
    ...
    return linkedMapOf<String, Map<String, JsonElement>>()
        .apply {
...
private fun buildProxyGroups(
    ...
    groups +=
        serviceItems.filter(enabledItems::contains).mapNotNull(serviceGroupNames::get).map {
            serviceName ->
            buildServiceSelectGroup(name = serviceName, regionNames = regionNames)
        }
...
private fun buildRules(enabledItems: Set<OverridePresetItem>): List<String> {
    return buildList {
        ...
        serviceItems.filter(enabledItems::contains).forEach { item ->
...
private fun buildAutoGroup(name: String, filter: String? = null): Map<String, JsonElement> {
...
private fun buildRegionAutoGroup(region: OverridePresetRegion): Map<String, JsonElement> {
    return buildAutoGroup(name = region.groupName, filter = region.filter)
}

private fun buildProxySelectGroup(regionNames: List<String>): Map<String, JsonElement> {
    return linkedMapOf(
...
private fun buildServiceSelectGroup(
    ...
    return linkedMapOf(
        ...
        "type" to JsonPrimitive("select"),
        "proxies" to jsonArrayOf(listOf("Proxy", "DIRECT", "Auto").plus(regionNames).distinct()),
    )
}

private fun buildRuleSetRule(providerId: String, target: String, noResolve: Boolean): String {
    return if (noResolve) {
...
private fun safeContentOrNull(primitive: JsonPrimitive): String? =
