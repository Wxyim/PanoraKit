...
object Clash {
    ...
    fun compilePreview(request: CompileRequest): CompileResult {
        val payload =
            Bridge.nativeCompilePreview(Json.encodeToString(CompileRequest.serializer(), request))
        return Json.decodeFromString(CompileResult.serializer(), payload)
    }

    fun compileToFile(request: CompileRequest): CompileResult {
        val payload =
            Bridge.nativeCompileToFile(Json.encodeToString(CompileRequest.serializer(), request))
        return Json.decodeFromString(CompileResult.serializer(), payload)
    }
    ...
    fun startLocalProxyHttpListener(listenAt: String): String? {
        return startHttp(listenAt)
    }

    fun stopLocalProxyHttpListener() {
        stopHttp()
    }
    ...
    fun inspectCompiledConfig(yamlText: String): ConfigurationOverride? {
        ...
        return runCatching { Json.decodeFromString(ConfigurationOverride.serializer(), configJson) }
            .getOrElse { error ->
