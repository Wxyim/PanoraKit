 ...
 *
 * Rules with types unsupported by mihomo/Clash are dropped. Surge/Loon-specific
 * aliases and built-in REJECT variants are normalized to Clash equivalents.
 */
internal object PluginRuleParser {

    /**
     * Surge-only rule types with no equivalent in mihomo/Clash.
     * Lines using these types are dropped during import.
     */
    private val UNSUPPORTED_RULE_TYPES = setOf(
        "USER-AGENT",
        "URL-REGEX",
        "HEADER",
        "HTTP-METHOD",
    ...
    /**
     * Surge rule type aliases -> mihomo/Clash rule type names.
     * Applied before writing the normalized rule.
     */
    private val RULE_TYPE_ALIASES = mapOf(
        "DEST-PORT" to "DST-PORT",
    )

    /**
     * Surge built-in REJECT variants with no direct equivalent in Clash.
     * Normalized to plain REJECT.
     */
    private val SURGE_REJECT_VARIANTS = setOf(
        "REJECT-TINYGIF",
        "REJECT-200",
        "REJECT-IMG",
        "REJECT-DICT",
    ...
    /**
     * Parse [text] and return all rules from the [Rule] section that are
     * compatible with mihomo/Clash.
     *
     * Supports both direct rule lines and keyed rule fields like
     * RULE-1 = DOMAIN-SUFFIX,example.com,REJECT used in some Loon plugins.
     *
    ...
    fun parseRules(text: String): List<String> {
        ...
        for (rawLine in text.lineSequence()) {
            ...
            if (!inRuleSection || line.isEmpty() || line.startsWith('#') || line.startsWith(';')) continue
            ...
            if (policyIndex < normalizedParts.size &&
