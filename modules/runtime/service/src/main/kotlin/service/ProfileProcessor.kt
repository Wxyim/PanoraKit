...
import com.github.nomadboxlab.monadbox.core.NetworkConstants
...
import com.github.nomadboxlab.monadbox.service.runtime.util.processingDir
                ...
                httpClient.newCall(request).execute().use { response ->
                    ...
                    targetFile.outputStream().use { output ->
                        ...
                        while (inputStream.read(buffer).also { bytesRead = it } != -1) {
                            ...
                            if (totalBytesRead > MAX_SUBSCRIPTION_BYTES) {
                                error(
                                    "Subscription response exceeds ${MAX_SUBSCRIPTION_BYTES / 1024 / 1024} MiB limit"
                                )
                            }
        ...
        withContext(Dispatchers.IO + NonCancellable) {
            processLock.withLock {
                ...
                if (snapshot.type == Profile.Type.Url) {
                    subInfo =
                        fetchUrlSubscription(context, snapshot.uuid, snapshot.source) { progress ->
                            try {
                                cb?.updateStatus(
                                    com.github.nomadboxlab.monadbox.core.model.FetchStatus(
                                        action =
                                            com.github.nomadboxlab.monadbox.core.model.FetchStatus
                                                .Action
                                                .FetchConfiguration,
                ...
                Clash.fetchAndValid(context.processingDir, snapshot.source, false) {
                        try {
                            cb?.updateStatus(it)
                        } catch (e: Exception) {
                            cb = null
