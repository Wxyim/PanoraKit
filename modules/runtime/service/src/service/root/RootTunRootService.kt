...
class RootTunRootService : RootService() {
    ...
    private val binder = object : IRootTunService.Stub() {
        override fun startRootTun(requestJson: String): String {
            ...
            val spec = runCatching { createSpec("start") }.getOrElse { error ->
                val message = error.runtimeGatewayMessage("root tun spec build failed")
                startupLogStore.append("ROOT_TUN root-service: binder branch=start specFailed=$message")
                return encodeFailure(RuntimeGatewayErrorCode.RUNTIME_SPEC_BUILD_FAILED, message)
            }
            ...
            if (previousSpec == null) {
                startupLogStore.append("ROOT_TUN root-service: rollback skipped reason=no_snapshot")
            ...
            startupLogStore.append("ROOT_TUN root-service: rollback begin profile=${previousSpec.profileUuid}")
            val rollback = runSuspendBlocking { runtime.start(previousSpec) }
            if (rollback.success) {
                startupLogStore.append("ROOT_TUN root-service: rollback done profile=${previousSpec.profileUuid}")
                return encodeFailure(
                    failedCode,
            ...
            val rollbackMessage = rollback.error?.takeIf { it.isNotBlank() }
        ...
        override fun restartRootTun(requestJson: String): String {
            ...
            val spec = runCatching { createSpec("restart") }.getOrElse { error ->
                return encodeFailure(
        ...
        override fun reloadActiveProfile(requestJson: String): String {
            ...
            val spec = runCatching { createSpec("reload") }.getOrElse { error ->
                return encodeFailure(
                    RuntimeGatewayErrorCode.RUNTIME_SPEC_BUILD_FAILED,
                    error.runtimeGatewayMessage("root tun spec build failed"),
                )
        ...
        override fun queryProxyGroupJson(name: String, sort: String): String? {
            return RootTunJson.encode(runtime.queryProxyGroup(name, com.github.yumelira.yumebox.core.model.ProxySort.valueOf(sort)))
        }
        ...
        override fun queryProvidersJson(): String =
            RootTunJson.encode(runtime.queryProviders())
        ...
        override fun healthCheck(group: String): String? = runSuspendBlocking { runtime.healthCheck(group) }

        override fun healthCheckProxy(proxyName: String): String = runSuspendBlocking { runtime.healthCheckProxy(proxyName) }

        override fun updateProvider(type: String, name: String): String? = runSuspendBlocking { runtime.updateProvider(type, name) }
        ...
        override fun queryRecentLogsJson(sinceSeq: Long): String {
            return RootTunJson.encode(RootTunLogChunk.from(runtime.queryRecentLogsJson(sinceSeq)))
        }
    }
    ...
    private fun loadLastGoodRuntimeSpec(): RuntimeSpec? {
        ...
        return runCatching {
            ...
            if (decoded.runtimeConfigPath.isNotBlank()) {
                ...
                decoded.copy(runtimeConfigPath = java.io.File(decoded.profileDir).resolve("runtime.yaml").absolutePath)
            }
        }
    }
    ...
    private fun RuntimeSnapshot.toRootTunStatus(): RootTunStatus {
        ...
        val parsedMessage = if (parsedCode != null) {
